...
`alloc.go` 针对 64 KB 以内的帧实现了分层分配器：17 个 `sync.Pool` 分别缓存 2^n 容量的切片，`msb()` 函数利用 De Bruijn 序列常数在 O(1) 时间内定位到最合适的池子。复用出来的切片已经被零填充过，流可以直接写入而无需再次分配或擦除。

这样带来的系统收益包括：

1. 单次分配的浪费率 < 50%，在高并发下也能准确控制会话级缓冲占用。
2. 重复使用固定容量的切片显著降低 GC 压力，同时把内存清零的成本摊薄到多次收发。
